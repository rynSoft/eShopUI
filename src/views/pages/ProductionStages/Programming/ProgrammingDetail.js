...
function ProgrammingDetail(props) {
  ...
  const loadData = () => {
    axios
      ...
      .then((response) => {
          setDataLeft(response.data.data.notRead.filter((x) => x.programmingBeginDate == null));
          setDataRight(response.data.data.read);

          response.data.data.read.filter((x) => x.programmingElapsedTime == null && x.programmingBeginDate != null).length > 0
  ...
  //       let resDataLeft = resData.filter((x) => x.programmingBeginDate == null);
  //       setDataLeft(resDataLeft);
  //       let resDataRight = resData.filter((x) => x.programmingBeginDate != null);
  //       setDataRight(resDataRight);

  //       resDataRight.filter((x) => x.programmingElapsedTime == null && x.programmingBeginDate != null).length > 0
  //         ? setFinishStateButton(true)
  //         : setFinishStateButton(false);
  ...
  const updateProgramData = (datas) => {
    axios
      .put(
        process.env.REACT_APP_API_ENDPOINT + "api/ProductionProcessManual/ProgrammingUpdate", datas
      )
      .then((res) => {
        if (res.data.success) {
          loadData();
  ...
  const vardiyaTamamla = () => {
    if (readerState) {
      let datas =
      {
        ...
        programmingBeginDate: new Date().toISOString(),
      };
    ...
    if (readerState) {
      ...
      .get(
        process.env.REACT_APP_API_ENDPOINT +
            ...
            +"&process=9"+"&previousProcess=" + previousProcess
      )
      .then((response) => {
  ...
  //       (obj) => obj.qrCode === e.replace("ç", ".").replace("ç", ".").replace("ç", ".") && obj.programmingBeginDate == null
  //     );
    ...
    <Fragment><Row>
      ...
      <Col xl="12" md="12" xs="12" style={{ minHeight: 100 }}>
        <TimerCalculate
          ...
          PproductionProcess={9}
          screenName="Programlama"
            ...
            {tableState ? (
              <Table responsive style={{ marginTop: 10 }} size="sm">
                <thead>
                  <tr>
                    <th>QrCode</th>
                    <th>Program Baslangıc Tarihi</th>
                    <th>Program Bitiş Tarihi</th>
                    <th>Gecen Süre</th>
                    <th>Kullanıcı</th>
